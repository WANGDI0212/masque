...
$MASQUEDIR/vsearch_bin/bin/vsearch --derep_fulllength $WORKDIR/${sample}_filt.fasta -output $WORKDIR/${sample}_drep.fasta -sizeout
# Remove singleton
...
# Clustering
$MASQUEDIR/vsearch_bin/bin/vsearch --cluster_size $WORKDIR/${sample}_nochim.fasta --id 0.97 --centroids $WORKDIR/${sample}_otu.fasta --sizein --relabel OTU_
# Mapping
$MASQUEDIR/vsearch_bin/bin/vsearch -usearch_global $WORKDIR/${sample}_filt.fasta -db $WORKDIR/${sample}_otu.fasta   --id 0.97 -uc $WORKDIR/${sample}_map.txt
# Count matrix
python $MASQUEDIR/usearch_python_scripts/uc2otutab.py $WORKDIR/${sample}_map.txt > $WORKDIR/${sample}_otu_table.tsv
# Annotation MOCK
$MASQUEDIR/vsearch_bin/bin/vsearch --usearch_global $WORKDIR/${sample}_otu.fasta --db $DATADIR/HMP_MOCK_v35_annotated.fasta --id 0.9  --blast6out $WORKDIR/${sample}_vs_mock.tsv --alnout $WORKDIR/${sample}_vs_mock_ali.txt
